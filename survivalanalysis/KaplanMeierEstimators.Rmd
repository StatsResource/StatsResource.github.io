...
The estimator of the survival function $\LARGE S(t)$ (the probability that life is longer than $\LARGE t$) is given by:

\[{\LARGE {\widehat {S}}(t)=\prod \limits _{i:\ t_{i}\leq t}\left(1-{\frac {d_{i}}{n_{i}}}\right) =\prod \limits _{i:\ t_{i}\leq t}\left(1-\lambda_i \right),} \]

\noindent with $\LARGE t_{i}$ a time when at least one event happened, $\LARGE d_i$ the number of events (e.g., deaths) that happened at time $\LARGE t_{i}$, and 
$\LARGE n_{i}$ the individuals known to have survived (have not yet had an event or been censored) up to time $\LARGE t_{i}$. 
...
on data from the 12 insects.
 ...
 ||t (weeks) | $\LARGE S(t)$ | |
 |-------------|-------------|--------------|----------|
||$\LARGE 0 \leq t < 1$ | 1.0000 ||
||$\LARGE 1 \leq t < 3$ | 0.9167||
||$\LARGE 3 \leq t < 6$ | 0.7130||
||$\LARGE t \geq 6$ | 0.4278 ||
