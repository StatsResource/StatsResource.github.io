...
**The Geometric Distribution: A Quick Tutorial**

The geometric distribution is a discrete probability distribution that models the number of independent Bernoulli trials needed to achieve the first success. A Bernoulli trial is a random experiment with only two possible outcomes: success or failure.

**Key Characteristics:**

* **Success Probability (p):** The probability of success on any single trial.
* **Number of Trials (X):** The random variable representing the number of trials until the first success.

**Probability Mass Function (PMF):**

The probability of achieving the first success on the x-th trial is given by:
...
Consider flipping a fair coin repeatedly until you get heads. The probability of getting heads on any single flip is 0.5. The probability of getting heads for the first time on the third flip is:

P(X = 3) = (1 - 0.5)^(3-1) * 0.5 = 0.25

**Applications:**

The geometric distribution has various applications in fields like:

* **Reliability Engineering:** Modeling the number of attempts needed to successfully start a machine.
* **Quality Control:** Determining the number of items inspected until a defective one is found.
* **Sports:** Calculating the probability of a basketball player making their first free throw on a given attempt.

**In Summary:**

The geometric distribution is a valuable tool for modeling the number of trials needed to achieve a single success in a series of independent Bernoulli trials. Understanding its properties can help in solving various real-world problems.
